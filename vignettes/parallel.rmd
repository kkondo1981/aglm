...
We can set `parallel.alpha=TRUE` to parallelize the outer loop (the loop for \eqn{\alpha}).
...
To make sure that both results are identical, we will compare means of `cvm` for all \eqn{\alpha}.
