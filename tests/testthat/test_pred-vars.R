...
test_that("Check the creation of PredVars objects when both x and x_UD are specified.", {
  ...
  expect_equal(length(res@vars_info), nvar + nvar_OD)
  expect_equal(res@vars_info[[4]]$idx, 4)
  expect_equal(res@vars_info[[4]]$name, paste0(var_names[1], "_OD"))
  expect_equal(res@vars_info[[4]]$data_column, 1)
  expect_equal(res@vars_info[[4]]$type, "O")
  expect_equal(res@vars_info[[3]]$idx, nvar)
  expect_equal(res@vars_info[[3]]$name, paste0(var_names[3], "_UD"))
...
test_that("Check the creation of PredVars objects when only x_UD is specified.", {
  ...
  expect_equal(res@vars_info[[1]]$name, paste0(var_names[1], "_UD"))
...
test_that("Check the creation of PredVars objects when only x is specified.", {
  ...
  expect_true("PredVars" %in% class(res))
  expect_equal(dim(res@data), c(nobs, nvar))
  expect_equal(length(res@vars_info), nvar * 2)
  expect_equal(res@vars_info[[3]]$idx, 3)
  expect_equal(res@vars_info[[3]]$name, paste0(var_names[1], "_OD"))
  expect_equal(res@vars_info[[3]]$data_column, 1)
  expect_equal(res@vars_info[[3]]$type, "O")
})
...
test_that("Check getDesignMatrix().", {
  ...
  expect_equal(dim(dm), c(nobs, nvar_OD + 9 + nvar_OD * 20))
  expect_equal(sort(unique(as.numeric(dm[, -(1:nvar_OD)]))), c(0, 1))
  expect_equal(colnames(dm)[1:2], c("Var_1", "Var_2"))
})
