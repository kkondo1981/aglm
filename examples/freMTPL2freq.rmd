...
The response values contain too large values as claim numbers in less than one year.
...
# Elastic Net with Poisson family

We use glmnet library to fit the data to elastic net with poisson family.

## Preprocess

Because glmnet doesn't accept qualitative variables, we need to convert them into dummy variables.

```{r}
expandX <- function(x) {
  xx <- x %>% keep(is.numeric)
  for (nm in colnames(x)) {
    v <- x[[nm]]
    if ("factor" %in% class(v)) {
      m <- getUDummyMatForOneVec(v, drop_last=FALSE)$dummy_mat
      colnames(m) <- sprintf("%s_UD_%d", nm, seq(ncol(m)))
      xx <- cbind(xx, m)
    }
  }
  as.matrix(xx)
...
# Create x with dummies
dum_x <- expandX(x)
dum_x_train <- dum_x[-test.id,]
dum_x_test <- dum_x[test.id,]
```


## Choose hyperparameters

The elastic net has two hyperparameters $\alpha$ and $\lambda$.
...
We can choose these hyperparameters by cross-validation using `glmnetUtils::cva.glmnet()`.
An R code for this purpose is as below, but commented out it because this process is time-consuming.

```{r}
# cva.model <- cva.glmnet(dum_x_train, y_train, family="poisson", trace.it=TRUE)
# alpha.min.index <- which.min(lapply(cva.model$modlist, function(mod){min(mod$cvm)}))
# alpha.min <- cva.model$alpha[alpha.min.index]
# lambda.min <- cva.model$modlist[[alpha.min.index]]$lambda.min
# 
...
Instead, use precalculated values here as below.

```{r}
alpha.min <- 0.34300000000000008260059303211164660751819610595703125
lambda.min <- 0.0001797932830553665580451683236873350324458442628383636474609375
```
...
```{r}
best_model <- glmnet(dum_x_train, y_train, offset=off_train, family="poisson", lambda=lambda.min, alpha=alpha.min, trace.it=TRUE)
```
...
```{r}
y_pred <- predict(best_model, newx=dum_x_test, newoffset=off_test, type="response")
```
...
We can see which coefficients are used by the model, by calling `glmnet::coef()` as below.

```{r}
cfs <- coef(best_model)
keys <- rownames(cfs)[-1]  # Cut off the intercept (index=1)
values <- cfs[,1][-1] # Cut off the intercept (index=1)
df <- data.frame(keys=keys, values=values)
g <- ggplot(df, aes(x=keys, y=values)) +
  geom_bar(stat="identity") +
  coord_flip() +
  scale_x_discrete(limits=rev(df$keys))
plot(g)
```
...
```{r}
glmnet.result <- list(alpha.min=alpha.min,
                      lambda.min=lambda.min,
                      best_model=best_model,
                      y_pred=y_pred,
                      dev_test=dev_test)
save(glmnet.result, file="glmnet_result.rda")
```


# AGLM wiht Poisson family

## Subsampling to make computation enable
...
### Understand the model
```{r}
plot(best_model, verbose=FALSE, ask=FALSE, resid=FALSE, layout=c(2, 2))
```
...
```{r}
aglm.result <- list(alpha.min=alpha.min,
                    lambda.min=lambda.min,
                    best_model=best_model,
                    y_pred=y_pred,
                    dev_test=dev_test)
save(aglm.result, file="aglm_result.rda")
```
