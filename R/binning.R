...
#'   Only used when `breaks` is not set.
#'
...
#' @export
#' @importFrom assertthat assert_that
executeBinning <- function(x_vec, breaks=NULL, nbin.max=100, allow_na=FALSE, method="freq") {
  # Check arguments
  ...
  if (is.ordered(x_vec)) {
    labels <- as.integer(rep(NA, length(x_vec)))
